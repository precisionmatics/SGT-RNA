"""
RNA-PDFL · Step 11: Interface-Focused PDFL

Key idea: current PDFL uses all RNA pocket atoms within 12Å of the ligand.
Here we recompute PDFL on RNA atoms within {4, 6, 8}Å of ANY ligand atom —
...
log.info("=" * 70)
log.info("RNA-PDFL · Step 11: Interface-Focused PDFL")
log.info("=" * 70)

# ── PDFL constants (identical to step03/04) ───────────────────────────────────
...
# ── PDFL helper functions (from step03) ───────────────────────────────────────
...
def interface_pdfl(rna_coords, lig_coords, cutoff):
    """Compute 3,600-dim PDFL on RNA atoms within `cutoff` Å of any ligand atom."""
...
# ── compute interface PDFL for all complexes × all cutoffs ────────────────────
log.info(f"\nComputing interface PDFL ({len(CUTOFFS)} cutoffs × {FEATS_PER_CUTOFF} feats) ...")
n = len(records)
...
for idx, rec in enumerate(records):
    ...
    for cutoff in CUTOFFS:
        parts.append(interface_pdfl(rec["rna_coords"], rec["lig_coords"], cutoff))
...
log.info("\n" + "=" * 70)
log.info(f"  COMBINED r (step11 interface PDFL) = {combined_r:.4f}")
...
ax.set_title(f"Step 11: Interface PDFL  (combined r={combined_r:.4f})", fontsize=12, fontweight="bold")
